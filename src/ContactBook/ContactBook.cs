...
public class ContactBook
{
    ...
    {
        Console.Clear();
        Console.WriteLine("Thank you for using the Contact Book. Goodbye!");
    }
    ...
    private void UpdateContact(int index)
    {
        ...
        if(Confirm("Do you want to update the phone number?", NO))
        {
            Console.Write("Enter Phone: ");
            phone = Console.ReadLine()!;
        }

        if(Confirm("Do you want to update the email address?", NO))
        {
            Console.Write("Enter Email: ");
    ...
    private void FindContacts()
    {
        ...
        if(Confirm("Do you want to search contacts?", YES))
        {
            filteredContacts = allContacts.FindAll(c => 
             ...
             page = 1;
            Console.WriteLine("Contacts search completed.");
        }

        else
        {
            Console.WriteLine("Contacts search cancelled.");
        }
        PressEnterContinue();
    }
    ...
    private void DeduplicateContacts()
    {
        ...
        foreach(var group in duplicateGroups)
        {
            if(group.Count > 1)
            {
                ...
                {
                    temp.AddRange(group);
                    Console.WriteLine("Contacts merge cancelled.");
                }
                Console.WriteLine();
                PressEnterContinue();
            }
        ...
        if(Confirm("Do you want to apply the merges to the contact list?", NO))
        {
